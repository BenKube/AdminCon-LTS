...
/* AdminCon 8.0 Command Line Interface Edition - Source Code - DesktopEnvironment.cs
...
namespace AdminCon_CLI_dotnetEdition.Modules.ACDesktop.Components
{
    public partial class DesktopEnvironment : Form
    {
        ...
        {
            Process.Start(ProgramLocator.GeneratePath()+"\\PIZZACAKE.EXE");
        }
        private void internetInformationServiceToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        private void backgroundToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            OpenFileDialog wallpaperDialog = new OpenFileDialog();
            wallpaperDialog.Filter = "Image Files (.png)|*.png|Image Files (.jpg)|*.jpg|All Files (*.*)|*.*";
            if (wallpaperDialog.ShowDialog().Equals(DialogResult.OK)) this.BackgroundImage = new Bitmap(wallpaperDialog.FileName);
        ...
        private void timeTicker_Tick(Object sender, EventArgs e)
        {
            ...
            this.cpuUsageDynamicChart.Series["Memory Usage"].Points.AddXY(elapsedtime, Math.Round(PerfMon.Memory_Usage/(new ComputerInfo().TotalPhysicalMemory/1024), 3)*100);
            this.cpuUsageLabel.Text = Math.Round(PerfMon.CPU_Usage,3)+ "%";
            this.memUsageLabel.Text = Math.Round(PerfMon.Memory_Usage / (new ComputerInfo().TotalPhysicalMemory / 1024), 3) * 100 + "%, "+
                Math.Round(PerfMon.Memory_Usage/1024, 3)+"MB";
            ...
            if (this.cpuUsageDynamicChart.Series["Memory Usage"].Points.Count > 400)
            {
                this.cpuUsageDynamicChart.Series["Memory Usage"].Points.Clear();
            }
        }
